...
├•͜͡➣ ║▌│█║▌│ █║▌│█║▌║
├•͜͡➣ 🍁⃧‣₁ ₈⃜ ₅₃⃜ ₆ ₇⃜₄₂⃜ ₅ ₁⃜₈🎼🎬🎞️🎮
├•͜͡➣ (๑ ิټ ิ) 🅒︎🅡︎🅔︎🅐︎🅓︎🅞︎🅡︎
├•͜͡➣ ️♆᭄🌝᭄ıllıllıʝơʂɛ℘ɧıllıllı♆᭄🌚᭄
│
╰─────────────͜͡🔥̷̷̶̸̸͙̽̿͆ͤͫͫ͐͜ͅ
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				break*/
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'por fin te fuiste nadie te queria aki kk jajaja 😂👋 :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						...
						mentions(`por fin te fuiste nadie te queria aki kk jajaja 😂👋 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
		...
		if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('A quien quieres sgg?')
					if (args[0].startsWith('08')) return reply('Use el codigo de pais')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Fallo al agg , talvez porque este en privado')
					}
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
